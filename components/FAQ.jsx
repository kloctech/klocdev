...
function Chevron({ open }) {
  return (
    <svg className={`h-5 w-5 shrink-0 transition-transform ${open ? "rotate-180" : ""}`}
...
function FaqItem({ q, a, open, onToggle }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0b1220] px-6 py-5 text-slate-200">
      <button className="flex w-full items-center justify-between gap-6 text-left"
      ...
      {open && <div className="pt-3 text-sm leading-relaxed text-slate-400">{a}</div>}
    </div>
  );
}
...
export default function FAQ({ variant = "embedded", className = "" }) {
 ...
 <section
  ...
  >
      ...
      <h1 className={`mb-8 text-center font-bold tracking-tight ${isStandalone ? "text-4xl sm:text-5xl" : "text-2xl sm:text-3xl"}`}>
      ...
      {isStandalone && (
        <div aria-hidden className="pointer-events-none absolute inset-x-0 -bottom-16 -z-10 h-40 bg-gradient-to-t from-violet-700/20 to-transparent blur-2xl" />
      )}
    </section>
  );
}
