...
import { Eye, ExternalLink } from "lucide-react"; // 👈 icons
...
export default function ProjectsSection() {
  return (
    <section className="mx-auto max-w-7xl px-4 !pt-0 !mt-0">
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 overflow-visible">
        {PROJECTS.map((p, i) => {
          const row = Math.floor(i / 3);
          let offsetClass = "xl:translate-x-0";
          if (row === 0) offsetClass = "xl:-translate-x-6";
          else if (row === 1) offsetClass = "xl:translate-x-6";

          return (
            <article
              key={p.name}
              className={`group rounded-2xl border border-white/10 bg-slate-900/50 p-4
                          shadow-[0_8px_30px_rgba(2,6,23,0.35)]
                          backdrop-blur transition-transform duration-300 hover:-translate-y-1 ${offsetClass}`}
            >
              {/* Header: label + icon on the left, “View” + eye icon on the right */}
